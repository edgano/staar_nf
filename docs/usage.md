# nf-core/associations: Usage

## :warning: Please read this documentation on the nf-core website: [https://nf-co.re/associations/usage](https://nf-co.re/associations/usage)
...
```console
nextflow run nf-core/associations --input samplesheet.csv --genome GRCh37 -profile docker
```
...
```console
nextflow pull nf-core/associations
```
...
First, go to the [nf-core/associations releases page](https://github.com/nf-core/associations/releases) and find the latest version number - numeric only (eg. `1.3.1`). Then specify this when running the pipeline with `-r` (one hyphen) - eg. `-r 1.3.1`.
